...
if ($pharmacy) {
    ...
    try {
        $medicines_table_exists = $conn->query("SHOW TABLES LIKE 'medicines'");
        $debug_info[] = "medicines table exists: " . ($medicines_table_exists && $medicines_table_exists->num_rows > 0 ? 'Yes' : 'No');
        
        if ($medicines_table_exists && $medicines_table_exists->num_rows > 0) {
            if ($system_status === 'new' && isset($pharmacy['id'])) {
                // Get total medicines count
                $stmt = $conn->prepare("SELECT COUNT(*) as count FROM medicines WHERE pharmacy_id = ?");
                if ($stmt) {
...
$low_stock_alerts = [];
if ($pharmacy && $system_status === 'new' && isset($pharmacy['id'])) {
    try {
        $stmt = $conn->prepare("
